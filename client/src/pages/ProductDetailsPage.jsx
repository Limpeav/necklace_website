...
const ProductDetailsPage = () => {
  ...
  if (loading) {
    return <section className="container-shell py-20"><div className="h-[600px] animate-pulse rounded-[2rem] bg-white/5" /></section>;
  }

  if (error || !data) {
    return <section className="container-shell py-20"><p className="rounded-2xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-200">{error || "Product not found"}</p></section>;
  }
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_0.95fr]">
        <div className="grid gap-4 sm:grid-cols-2">
          {product.images.map((image) => (
            <FallbackImage key={image} src={image} alt={displayId} className="h-[360px] w-full rounded-[2rem] object-cover" />
          ))}
        </div>
        ...
        <div className="surface-card p-8 md:p-10">
          ...
          {product.description && product.description !== displayId && (
            <p className="mt-6 text-sm leading-8 text-stone-300">{product.description}</p>
          )}
